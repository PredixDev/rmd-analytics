        ...
        implements IRefAppAnalytic
{
            ...
            Map<String, FieldData> calculatedOutputMap)
    {
        ...
        try
        {
            putResponse = this.restClient.post(this.fdhRestConfig.getPutFieldDataEndPoint(), putRequest, headers,
    ...
    private Map<String, FieldData> resolveData(RunAnalyticRequest request, List<Header> headers)
    {
        ...
        log.info("Get Field Data ===>Request........................." + getRequest1);
        ...
        try
        {
            getFieldDataResponse = this.restClient.post(

                    this.fdhRestConfig.getGetFieldDataEndPoint(), getRequest, headers,
                    this.restConfig.getDefaultConnectionTimeout(), this.restConfig.getDefaultSocketTimeout());
        ...
        {
            if ( getFieldDataResponse != null ) try
            {
                getFieldDataResponse.close();
            }
            catch (IOException e)
            {
                throw new RuntimeException(e);
    ...
    private Map<String, FieldData> callAnalytic(RunAnalyticRequest serviceRequest, Map<String, FieldData> inputMap)
    {
        ...
        DatapointsResponse tsData = inputMap.get(PORT_NAME_TS_DATA) != null
                ? ((DatapointsResponse) inputMap.get(PORT_NAME_TS_DATA).getData()) : null;
        DataEvent alarmHiHiData = inputMap.get(PORT_NAME_ALARM_HIHI) != null
                ? ((OsaData) inputMap.get(PORT_NAME_ALARM_HIHI).getData()).getDataEvent() : null;
        DataEvent alarmLoLoData = inputMap.get(PORT_NAME_ALARM_LOLO) != null
                ? ((OsaData) inputMap.get(PORT_NAME_ALARM_LOLO).getData()).getDataEvent() : null;
        DataEvent alarmHiData = inputMap.get(PORT_NAME_ALARM_HI) != null
                ? ((OsaData) inputMap.get(PORT_NAME_ALARM_HI).getData()).getDataEvent() : null;
        DataEvent alarmLoData = inputMap.get(PORT_NAME_ALARM_LO) != null
                ? ((OsaData) inputMap.get(PORT_NAME_ALARM_LO).getData()).getDataEvent() : null;
        ...
        for (Port aPort : serviceRequest.getOutputPort())
        {
            ...
            {
                outputMap.put(ALARM_THRESHOLDLEVEL,
                        createFieldData(aPort.getFieldSelection(), thresholdLevelDataEvent));
            }
        }
    ...
    private void calculateAlarmData(SDDataEvent sdDataEvent, DataEvent alarmStatusDataEvent,
            ...
            DataEvent alarmLevelValueDataEvent, DataEvent alarmLevelValueTimeDataEvent,
            DatapointsResponse actualValueSeq, DataEvent alarmHIHISeq, DataEvent alarmLOLOSeq, DataEvent alarmHISeq,
            DataEvent alarmLOSeq)
    {
        ...
        if ( actualValueSeq != null )
        {
            ...
            log.info("List of Values = " + actualValueSeq);
            List<Double> values = convertObjectListToDoubleList(
                    actualValueSeq.getTags().get(0).getResults().get(0).getValues());
            log.info("List of Values = " + values);
            double actualValue = values.get(1);
            double actualValueOccuredTime = values.get(0);
            ...
            if ( !hihiValue.isNaN() && actualValue >= hihiValue.doubleValue() )
            {
                ...
                ((DMReal) thresholdDiffDataEvent).setValue((hihiValue.doubleValue() - actualValue) / thresholdDivisor);
            ...
            else if ( !hiValue.isNaN() && actualValue >= hiValue.doubleValue() )
            {
                ...
                ((DMBool) alarmStatusDataEvent).setValue(true);
                ((DAString) alarmLevelDataEvent).setValue(alarmMessage);
                ((DMReal) thresholdDiffDataEvent).setValue((hiValue.doubleValue() - actualValue) / thresholdDivisor);
                ((DAString) thresholdLevelDataEvent).setValue(alarmMessage);
                ((DMReal) alarmLevelValueDataEvent).setValue(actualValue);
                ((DMReal) alarmLevelValueTimeDataEvent).setValue(actualValueOccuredTimeWithxaxisDeltaTime);
            }
            else if ( !loValue.isNaN() && actualValue <= loValue.doubleValue() )
            {
                ...
                ((DMBool) alarmStatusDataEvent).setValue(true);
                ((DAString) alarmLevelDataEvent).setValue(alarmMessage);
                ((DMReal) thresholdDiffDataEvent).setValue((loValue.doubleValue() - actualValue) / thresholdDivisor);
                ((DAString) thresholdLevelDataEvent).setValue(alarmMessage);
                ((DMReal) alarmLevelValueDataEvent).setValue(actualValue);
                ((DMReal) alarmLevelValueTimeDataEvent).setValue(actualValueOccuredTimeWithxaxisDeltaTime);
            }
            else if ( !loloValue.isNaN() && actualValue <= loloValue.doubleValue() )
            {
                ...
                ((DMBool) alarmStatusDataEvent).setValue(true);
                ((DAString) alarmLevelDataEvent).setValue(alarmMessage);
                ((DMReal) thresholdDiffDataEvent).setValue((loloValue.doubleValue() - actualValue) / thresholdDivisor);
                ((DAString) thresholdLevelDataEvent).setValue(alarmMessage);
                ((DMReal) alarmLevelValueDataEvent).setValue(actualValue);
                ((DMReal) alarmLevelValueTimeDataEvent).setValue(actualValueOccuredTimeWithxaxisDeltaTime);
            }
            else
            {
                ...
                if ( !loValue.isNaN() && !hiValue.isNaN() )
                {
                    ...
                    else
                    {
                        ...
                        ((DMReal) thresholdDiffDataEvent)
                                .setValue((actualValue - loValue.doubleValue()) / thresholdDivisor);
                    }
                }
                else if ( !loloValue.isNaN() && !hihiValue.isNaN() )
                {
                    if ( hihiValue.doubleValue() - actualValue < actualValue - loloValue.doubleValue() )
                        ...
                        ((DMReal) thresholdDiffDataEvent)
                                .setValue((actualValue - loloValue.doubleValue()) / thresholdDivisor);
                }
                ((DMReal) alarmLevelValueDataEvent).setValue(actualValue);
                ((DMReal) alarmLevelValueTimeDataEvent).setValue(actualValueOccuredTimeWithxaxisDeltaTime);
            }
            ...
            BigInteger bigInteger = BigInteger.valueOf(actualValueOccuredTimeWithxaxisDeltaTime);
            osacbmTime.setTimeBinary(bigInteger);
            numAlert.setLastTrigger(osacbmTime);
            numAlert.setAlertName(alarmMessage);
            sdDataEvent.getNumAlerts().add(numAlert);
        }
    }
    ...
    private List<Double> convertObjectListToDoubleList(List<Object> datapoints)
    {
        List<Double> dpList = new ArrayList<Double>();
        if ( !CollectionUtils.isEmpty(datapoints) )
        {
            for (Object obj : datapoints)
            {
                for (Object d : (List<Object>) obj)
                {
                    if ( d instanceof Long )
                    {
                        dpList.add(((Long) d).doubleValue());
                    }
                    else if ( d instanceof Integer )
                    {
                        dpList.add(((Integer) d).doubleValue());
                    }
                    else
                    {
                        dpList.add((Double) d);
